...
def generate_gnuplot_script(selected_data, styles, x_range, y_offset, show_legend):
...
if all_data:
    ...
    with st.sidebar.expander("🛠 フォント・目盛り・凡例", expanded=False):
        font_family = st.selectbox("フォント種類", ["Arial", "sans-serif", "serif", "monospace", "Times New Roman"])
        base_font_size = st.slider("基本文字サイズ", 8, 30, 18)
...
if selected_data:
    ...
    for i, d in enumerate(selected_data):
        ...
        fig_plotly.add_trace(go.Scatter(
            ...
            showlegend = True # 凡例リストには常に含めるが、全体のlayoutで制御
        ))
    ...
    fig_plotly.update_layout(
        xaxis=dict(
            title=dict(text="2-theta (deg.)", font=dict(size=base_font_size, family=font_family)),
            range=x_range, showgrid=show_grid, gridcolor='rgba(0,0,0,0.1)',
            tickfont=dict(size=tick_font_size, family=font_family)
        ),
        yaxis=dict(
            title=dict(text="Intensity (a.u.)", font=dict(size=base_font_size, family=font_family)),
            showgrid=show_grid, gridcolor='rgba(0,0,0,0.1)',
            tickfont=dict(size=tick_font_size, family=font_family)
        ),
        font=dict(family=font_family),
        showlegend=show_legend, # 【新機能反映】
    ...
    with c1:
        ...
        ax.set_xlim(x_range)
        ax.set_xlabel("2-theta (deg.)", fontsize=base_font_size)
    ...
    with c2:
        # gnuplotにも凡例設定を反映
        gp_script = generate_gnuplot_script(selected_data, individual_styles, x_range, y_offset, show_legend)
