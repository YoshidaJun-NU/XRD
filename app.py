...
# --- Sidebar (Settings) ---
st.sidebar.header("(1) ファイル読み込み設定")
...
2. ピーク検出してテーブル出力。
3. プロット＆ピーク表示。2に戻り，必要なピークを取捨選択
　　d値タブを押して予備的な相決定(ラメラ，hexagonal, tetragonal)
5. Colr相の決定
6. oblique相の決定
7. Cubic相の決定（別プログラム）
...
if df is not None:
    ...
    st.markdown("---")
    st.subheader("(5) 2D-Lattice Analysis (Reciprocal Space Q-plot)")
