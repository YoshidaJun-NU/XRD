...
st.title("🔬 XRD Multi-Plotter Pro (Enhanced Axes)")
...
if all_data:
    ...
    min_x_val = float(all_x.min())
    max_x_val = float(all_x.max())

    # 【新機能】手動入力とバーの併用
    c_in1, c_in2 = st.sidebar.columns(2)
    with c_in1:
        manual_min = st.number_input("Min X (2θ)", value=min_x_val, step=1.0)
    with c_in2:
        manual_max = st.number_input("Max X (2θ)", value=max_x_val, step=1.0)
    
    x_range = st.sidebar.slider("表示範囲 (バー操作)", min_x_val, max_x_val, (manual_min, manual_max), step=0.01)

    st.sidebar.header("5. Peak Settings")
...
if selected_data:
    ...
    for i, d in enumerate(selected_data):
        ...
        if use_peak_finder:
            ...
            fig_plotly.add_trace(go.Scatter(
                ...
                textfont=dict(size=tick_font_size, family=font_family),
    ...
    fig_plotly.update_layout(
        xaxis=dict(
            title=dict(text=x_label_html, font=dict(size=base_font_size, family=font_family)),
            range=x_range, showgrid=show_grid, gridcolor='rgba(0,0,0,0.1)',
            tickfont=dict(size=tick_font_size, family=font_family)
        ),
        yaxis=dict(
            title=dict(text=y_label_html if show_y_axis else "", font=dict(size=base_font_size, family=font_family)),
            showgrid=show_grid if show_y_axis else False,
            showticklabels=show_y_axis, # 【新機能】数値の表示/非表示
            ticks="outside" if show_y_axis else "", # 【新機能】目盛り線の表示/非表示
    ...
    # Matplotlib用のラベル（TeX記法で斜体θを表現）
    x_label_mpl = r'2$\theta$ (deg.)'
    ...
    with c1:
        ...
        ax.set_xlim(x_range)
        ax.set_xlabel(x_label_mpl, fontsize=base_font_size)
        
        if show_y_axis:
            ax.set_ylabel("Intensity (a.u.)", fontsize=base_font_size)
            ax.tick_params(axis='both', which='major', labelsize=tick_font_size)
        else:
            ax.set_yticklabels([]) # 数値を消す
            ax.tick_params(left=False) # 目盛り線を消す
        
        if show_grid:
            ax.grid(True, alpha=0.3)
        if show_legend:
            ax.legend(prop={'size': tick_font_size}, frameon=False)
